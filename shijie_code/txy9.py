...
[delta_t,delta_x,delta_y]=[0.008,0.00013,0.00056]
...
def task0(file):
    ...
    [pairs,xy]=ctry2.task_6(txy)#task
    return [txy,pairs,xy]
...
def pairs_index_5d(x):
    x=int(x)
    a=[]
    for i in range(x-4):
        for m in range(i+1,x-3):
            for n in range(m+1,x-2):
...
def nomalization_profile_4d():
    pairs_length=[file_length[i[0]]*file_length[i[1]]*file_length[i[2]]*file_length[i[3]] for i in indexs]
    N=sum(pairs_length)/sum([i**4 for i in file_length])
    return N

def nomalization_profile_5d():
    pairs_length=[pow(file_length[i[0]]*file_length[i[1]]*file_length[i[2]]*file_length[i[3]]*file_length[i[4]],1.5/5) for i in indexs]
    N=sum(pairs_length)/sum([i**1.5 for i in file_length])
    return N
...
def plot_g_5D_1():
    plt.clf()
    Z=hist_4d/xy_pool2*N
    X=np.histogramdd((xy_pool[0],xy_pool[1],xy_pool[2],xy_pool[3]),bins,range=[[0,delta_t/4],[0,delta_t/4],[0,delta_t/4],[0,delta_t/4]])[1][1][:-1]
    z00=[Z[i][i][i][i] for i in range(Z.shape[0])]
    plt.scatter(X,z00)
    plt.xlabel(r'$\delta T(s)$')
    plt.ylabel('g(x=y=z)')
    plt.grid()
...
def plot_g_5D_00_test(file_length_power):
...
def task_2_3():
    ...
    for n in x:
        ...
        y.append(plot_g_5D_00_test(file_length_power))
    plt.plot(x,y)
    plt.show()
    return
...
if __name__ == '__main__':
    ...
    files2=[i for i in os.listdir(path) if i[:5]=='data_' and os.path.getsize(path+i)>0]#100000
    files2.sort(key=takeSize)
    files2.reverse()
    files2=files2[:400]
    #indexs=pairs_index_4d(len(files2))
    indexs0=pairs_index_5d(10)
    indexs=indexs0
    for i in range(1,40):
    ...
    for p in results:
        ...
        xy_pool0.append(np.histogramdd((a[2][0],a[2][1],a[2][2],a[2][3]),bins,range=[[0,delta_t/4],[0,delta_t/4],[0,delta_t/4],[0,delta_t/4]])[0])
    file_length=[i.shape[0] for i in txy_pool]
    sum_0=sum([pow(i,1.5) for i in file_length])
    file_length_power=[pow(i,1.5)*len(files2)/sum_0 for i in file_length]
    z01=[]
    for n in xy_pool0:
        z01.append(np.array([n[i][i][i][i] for i in range(bins)]))
    X=np.histogramdd((xy_pool[0],xy_pool[1],xy_pool[2],xy_pool[3]),bins,range=[[0,delta_t/4],[0,delta_t/4],[0,delta_t/4],[0,delta_t/4]])[-1]
    print('Task2:'+str(len(indexs)))
    results=[pool.apply_async(ctry2.task_7, args=[txy_pool[i[0]],txy_pool[i[1]],txy_pool[i[2]],txy_pool[i[3]],txy_pool[i[4]]]) for i in indexs]
    #xy_pool2=np.array([[],[]])
    xy_pool2=np.zeros((bins,bins,bins,bins))
    ...
    #hist_g=np.histogram(pairs_g,bins,(0,hist_upper))
    hist_G=np.histogram(pairs_G,bins,(0,hist_upper))
    hist_4d=np.histogramdd((xy_pool[0],xy_pool[1],xy_pool[2],xy_pool[3]),bins,range=[[0,delta_t/4],[0,delta_t/4],[0,delta_t/4],[0,delta_t/4]])[0]
    N=nomalization_profile_5d()#N=nomalization_profile(txy_pool) This command is important
